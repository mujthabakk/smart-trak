...
import { setTheme as setThemeAction, toggleColorMode as toggleColorModeAction } from '@/store/slices/themeSlice'
...
export function ThemePanel({ open, onClose }: ThemePanelProps) {
  ...
  const { theme, colorMode } = useAppSelector((s) => s.theme)
  const setTheme = () => dispatch(setThemeAction())
  const toggleColorMode = () => dispatch(toggleColorModeAction())
