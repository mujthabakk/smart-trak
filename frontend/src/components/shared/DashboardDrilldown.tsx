...
const META: Record<DrillMetric, { title: string; subtitle: string; icon: typeof SchoolIcon; route: string }> = {
  schools: { title: 'Total Schools', subtitle: 'All registered schools on the platform', icon: SchoolIcon, route: '/super-admin/schools' },
  active: { title: 'Active Schools', subtitle: 'Schools with an active status', icon: CreditCard, route: '/super-admin/subscriptions' },
  expired: { title: 'Suspended / Pending', subtitle: 'Schools that need renewal attention', icon: AlertCircle, route: '/super-admin/subscriptions' },
  revenue: { title: 'Total Revenue', subtitle: 'Collected across the last 12 months', icon: DollarSign, route: '/super-admin/reports' },
  students: { title: 'Total Students', subtitle: 'Students enrolled across all schools', icon: Users, route: '/super-admin/schools' },
  drivers: { title: 'Total Drivers', subtitle: 'Drivers operating across all schools', icon: Bus, route: '/super-admin/schools' },
}
...
export function DashboardDrilldown({ metric, onClose }: { metric: DrillMetric | null; onClose: () => void }) {
  ...
  const schoolsQuery = useQuery({
    queryKey: ['schools', { pageSize: 200 }],
    queryFn: () => listSchools({ pageSize: 200 }),
    enabled: !!metric,
  })
  const allSchools = useMemo(() => schoolsQuery.data?.schools ?? [], [schoolsQuery.data])
  ...
  const allRevenueData = revenueQuery.data ?? []
  ...
  const rows = useMemo(() => {
    ...
    return list
  }, [metric, allSchools, plan, status, query])

  const revenueRows = useMemo(() => {
    const n = period === 'all' ? allRevenueData.length : Number(period)
    return allRevenueData.slice(-n)
  }, [period, allRevenueData])
  const revenueTotal = useMemo(() => revenueRows.reduce((sum, r) => sum + r.revenue, 0), [revenueRows])

  const metaValue = useMemo(() => {
    if (!metric) return ''
    if (metric === 'revenue') return formatCurrency(allRevenueData.reduce((sum, r) => sum + r.revenue, 0))
    if (metric === 'schools') return formatNumber(allSchools.length)
    if (metric === 'active') return formatNumber(allSchools.filter((s) => s.status === 'active').length)
    if (metric === 'expired') return formatNumber(allSchools.filter((s) => s.status === 'suspended' || s.status === 'pending').length)
    if (metric === 'students') return formatNumber(allSchools.reduce((sum, s) => sum + (s.student_count ?? 0), 0))
    return formatNumber(allSchools.reduce((sum, s) => sum + (s.driver_count ?? 0), 0))
  }, [metric, allSchools, allRevenueData])

  const isLoading = schoolsQuery.isLoading || revenueQuery.isLoading
          ...
          <>
            <DialogHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="flex items-center gap-2">{meta.title}<span className="text-[var(--primary)]">· {metaValue}</span></DialogTitle>
