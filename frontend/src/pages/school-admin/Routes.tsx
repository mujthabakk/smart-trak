...
export default function Routes() {
  ...
  // Route/stop assignment for students has no dedicated backend endpoint yet, so
  // drag-and-drop reassignment continues to be driven by local state — but that
  // state is now hydrated from the real students API instead of mock data.
  const [students, setStudents] = useState<Student[]>([])
  const studentsHydratedRef = useRef(false)
  useEffect(() => {
    if (!studentsHydratedRef.current && studentsQuery.data) {
