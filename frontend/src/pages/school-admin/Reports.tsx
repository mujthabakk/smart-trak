...
import type { AttendanceRecord, StatsCard as StatsCardData } from '@/types'
...
/** Inclusive list of yyyy-mm-dd date strings between `from` and `to`. */
...
/**
 * The attendance API only supports looking up a single `date` at a time, so
 * multi-day report views are built by fetching each day in the window and
 * merging the results (same pattern used in Attendance.tsx).
 */
async function fetchAttendanceForDates(dates: string[]): Promise<AttendanceRecord[]> {
  const results = await Promise.all(dates.map((d) => listAttendance({ date: d, pageSize: 1000 })))
...
export default function SchoolReports() {
  ...
  // ── School-scoped base data (real backend, JWT-scoped to this school) ────
  ...
  const schoolRoutes = useMemo(() => routesData?.routes ?? [], [routesData])

  const { data: driversData } = useQuery({
    queryKey: ['drivers'],
    queryFn: () => listDrivers(),
  })
  const schoolDrivers = useMemo(() => driversData?.drivers ?? [], [driversData])

  const { data: tripsData } = useQuery({
    queryKey: ['trips'],
    queryFn: () => listTrips(),
  })
  const allTrips = useMemo(() => tripsData?.trips ?? [], [tripsData])

  // Attendance for the standard-reports date range, fetched one day at a time
  // (see fetchAttendanceForDates above) and cached per [dateFrom, dateTo].
  const rangeDates = useMemo(() => datesBetween(dateFrom, dateTo), [dateFrom, dateTo])
  const { data: rangeAttendance } = useQuery({
    queryKey: ['attendance', 'range', dateFrom, dateTo],
    queryFn: () => fetchAttendanceForDates(rangeDates),
  })
  const allAttendance = useMemo(() => rangeAttendance ?? [], [rangeAttendance])
  ...
  // Attendance records for the selected day, fetched directly (single-date
  // lookup — no need for the multi-day merge helper used by the range query).
  const { data: dailyAttendanceData } = useQuery({
    queryKey: ['attendance', 'daily', dailyDate],
    queryFn: () => listAttendance({ date: dailyDate, pageSize: 1000 }),
  })
  const dailyAttendance = useMemo<AttendanceRecord[]>(
    () => dailyAttendanceData?.records ?? [],
    [dailyAttendanceData],
  )
