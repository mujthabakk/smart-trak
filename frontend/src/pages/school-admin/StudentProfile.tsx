...
export default function StudentProfile() {
  ...
  const route = useMemo(
    () => routes.find((r) => r.name === student?.route_name),
    [routes, student?.route_name],
  )
  const bus = useMemo(
    () => (route?.bus_id ? buses.find((b) => b.id === route.bus_id) : undefined),
    [buses, route],
  )

  // Attendance history for this student
  const attendanceQuery = useQuery({
    queryKey: ['attendance', { student_id: id }],
    queryFn: () => listAttendance({ student_id: id!, pageSize: 1000 }),
    enabled: !!id,
  })
  const records = useMemo(
    () =>
      [...(attendanceQuery.data?.records ?? [])].sort((a, b) => (a.date < b.date ? 1 : -1)),
