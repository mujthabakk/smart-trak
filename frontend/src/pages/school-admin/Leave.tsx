...
import { listLeave, updateLeave } from '@/lib/api/leave'
import { listStudents } from '@/lib/api/students'
import { formatDate, getInitials } from '@/lib/utils'
import type { Leave as LeaveType, LeaveStatus, Student } from '@/types'
...
function LeaveDetailDialog({ leave, students, onClose, onDecide }: DetailDialogProps) {
...
export default function Leave() {
  ...
  const schoolStudents = useMemo(() => studentsData?.students ?? [], [studentsData])

  const { data: leavesData, isLoading, isError } = useQuery({
    queryKey: ['leave'],
    queryFn: () => listLeave({ pageSize: 1000 }),
  })
  ...
  const decideMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: Exclude<LeaveStatus, 'pending'> }) =>
      updateLeave(id, {
        status,
      ...
      >
        ...
        <motion.div variants={item}>
          {isError && (
            ...
            >
              <AlertCircle size={16} className="flex-shrink-0 mt-0.5" /> Failed to load leave requests. Please try again.
            </div>
          )}
          {isLoading ? (
