...
export default function SchoolAdminDashboard() {
  ...
  // Bus/leave live-status data isn't covered by the reports (fleet-summary /
  // attendance-trend) endpoints, so it's fetched directly from the buses,
  // routes, drivers and leave list endpoints (all JWT-scoped to this school).
  const { data: busesData } = useQuery({
    queryKey: ['buses'],
    queryFn: () => listBuses(),
  })
  const allBuses = useMemo(() => busesData?.buses ?? [], [busesData])

  const { data: routesData } = useQuery({
    queryKey: ['routes'],
    queryFn: () => listRoutes(),
  })
  const allRoutes = useMemo(() => routesData?.routes ?? [], [routesData])

  const { data: driversData } = useQuery({
    queryKey: ['drivers'],
    queryFn: () => listDrivers(),
  })
  const allDrivers = useMemo(() => driversData?.drivers ?? [], [driversData])
  ...
  // Expiry alerts — buses + drivers (buses/drivers lists are already
  // JWT-scoped to this school by the backend, so no client-side filter needed)
