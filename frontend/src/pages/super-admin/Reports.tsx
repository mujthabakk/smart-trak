...
import { getRevenueReport, getSchoolGrowth } from '@/lib/api/reports'
...
export default function SuperAdminReports() {
  ...
  const schoolGrowthQuery = useQuery({
    queryKey: ['reports', 'school-growth'],
    queryFn: getSchoolGrowth,
  })
  const schoolGrowthData = schoolGrowthQuery.data ?? []

  const isLoading = schoolsQuery.isLoading || subscriptionsQuery.isLoading || revenueQuery.isLoading

  // ─── Revenue derivations ──────────────────────────────────────────────
  const totalRevenue = useMemo(
    () => revenueData.reduce((sum, m) => sum + m.revenue, 0),
    [revenueData],
  )
  ...
  // Revenue by top schools — aggregated from real subscription payments
  const topSchools = useMemo(() => {
    const bySchool: Record<string, { name: string; revenue: number }> = {}
    subscriptions.forEach((s) => {
      if (!bySchool[s.school_id]) bySchool[s.school_id] = { name: s.school_name, revenue: 0 }
      bySchool[s.school_id].revenue += s.amount_paid
    })
    return Object.values(bySchool)
      .map((s) => ({ name: s.name.length > 16 ? `${s.name.slice(0, 16)}…` : s.name, revenue: s.revenue }))
  ...
  const totals = useMemo(() => {
    const students = schools.reduce((s, x) => s + x.student_count, 0)
    const buses = schools.reduce((s, x) => s + x.bus_count, 0)
    const drivers = schools.reduce((s, x) => s + x.driver_count, 0)
          ...
          <Tabs defaultValue="revenue">
            ...
            <TabsContent value="revenue" className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <StatsCard title="Total Revenue" value={formatCurrency(totalRevenue)} change={momGrowth} icon={DollarSign} color="success" />
              ...
              <Card className="rounded-2xl overflow-hidden">
                ...
                <CardContent className="p-0">
                  <Table>
                    ...
                    <TableBody>
                      {[...revenueData].reverse().slice(0, 6).map((m) => (
            ...
            <TabsContent value="overview" className="space-y-6">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatsCard title="Total Revenue" value={formatCurrency(totalRevenue)} change={momGrowth} icon={DollarSign} color="success" />
                <StatsCard title="Total Schools" value={schools.length} icon={SchoolIcon} color="primary" />
                <StatsCard title="Total Students" value={formatNumber(totals.students)} icon={Users} color="info" />
                <StatsCard title="Active Subs" value={subscriptions.filter((s) => s.status === 'active').length} icon={CreditCard} color="warning" />
              </div>
              ...
              <Card className="rounded-2xl">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={280}>
                    <AreaChart data={schoolGrowthData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                      <defs>
                      ...
                      <Tooltip content={<ChartTooltip />} />
                      <Area type="monotone" dataKey="value" stroke="var(--primary)" strokeWidth={2} fill="url(#schoolsFill)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
